...
REPO_NAME="github.com/foss-for-synopsys-dwc-arc-processors/embarc_bsp.git"
...
rm -rf doc/build || die
cp ../doc.tar.gz doc || die
tar xzf doc/doc.tar.gz --directory doc|| die
rm -rf doc/doc.tar.gz || die
ls doc/build
git add --all || die
