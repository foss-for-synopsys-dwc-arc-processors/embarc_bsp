...
- **Drivers instantiation**: In embARC BSP, an object-oriented design is applied
  ...
  should be instantiated for every board.   For example, ``dw_gpio_obj.c`` and
  ``dw_gpio_obj.h`` are the instantiations of the DesignWare GPIO driver
  implementation in device abstraction layer, which contain the information of
  register address, interrupt number, etc.

- **Common board initialization process**: A standard bootup process is
  defined for all boards. The entry of this process is ``board_main`` defined
  in ``board/board.c``, and it calls to board initialization, fatfs and ntshell initialization,
  finally it will enter to ``main`` function if required.

- **Board resource unified definition**:  Most boards share common peripherals.
  Unified definitions are provided such as UART (as console interface), SD card interface, buttons, LEDs.
...
- **Linker script for boards**: Linker script is auto-generated from
  ...
  for both ARC GNU and MetaWare toolchains, located in ``option\toolchain`` folder. The files are named
...
- Board initialization steps - defined in ``board_init`` function.
  ...
  - (optional) Initialize peripherals.
  - Initialize other required board resources such as mux controller.

- (optional) setup xprintf library to provide printf functionality
- Go to ``main`` function
...
* AXS board support can be found in ``board/axs``, detailed in ARC AXS Software
  ...
  * :file:`common`: AXS board initialization functions
...
* EM Starter Kit (EMSK) support can be found in ``board/emsk``. It includes the following sub-folders.
  ...
  * :file:`common`: EMSK board initialization functions and board IO
  * :file:`drivers`: Various peripheral drivers for various examples
  * :file:`configs`: Different versions of emsk, and various core configurations for different version

* EM Software Development Package (EMSDP) support can be found in ``board/emsdp``. It includes the following sub-folders.

  * :file:`common`: EMSDP board initialization functions and board IO
  * :file:`drivers`: Various peripheral drivers for various examples
  * :file:`rev2/configs`: Various core configurations for different version
  * :file:`openocd`: support of openocd

* HS Development Kit (HSDK) board support can be found in ``board/hsdk``. It includes the following sub-folders.

  * :file:`common`: HSDK board initialization functions and board IO
  * :file:`drivers`: Various peripheral drivers for various examples
...
* IoT Development Kit (IoTDK) board support can be found in ``board/iotdk``. It includes the following sub-folders.
  ...
  * :file:`common`: IoTDK board initialization functions
  * :file:`drivers`: Various peripheral drivers for various examples
  * :file:`configs`: Support for different board versions (if applicable), and support for different core configurations for each board version (if applicable)

* nSIM virtual board support can be found in ``board/nsim``. It includes the following sub-folders.

  * :file:`common`: nSIM virtual board initialization functions
