...
There are a lot of examples and demos in embARC BSP which are located in
``<embarc>/example`` folder. General steps are shown below to build the
provided examples and demos, e.g. the example in
...
Here, we will show how to create a new application in embARC BSP,  taking a simple ``hello world`` as an example.

* Goals

    * Baremetal application based on embARC BSP
...
3. Change the configurations in makefile according to your hardware configuration and application.
    ...
    * Change the embARC BSP root: change the value of ``EMBARC_ROOT`` to
      ``../..``. ``EMBARC_ROOT`` can be a relative path or an absolute path.
...
* Open the ARC GNU IDE and set the workspace location to *ide_projects/emsk_22/gnu* in the embARC root. Set the workspace location in the right way.
...
* Build the projec in the IDE. When the build process is finished without any errors, open **Debug Configurations**, and create a debug configuration for this project. Remember to choose the correct ELF file, and set the right **Gdbserver Settings** in the **Debugger** tab.
